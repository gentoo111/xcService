...
public class MediaProcessTask {
     ...
     * 1.解析消息,得到媒资文件的id,查询数据库得到文件信息(文件路径)
     * 2.调用工具类将视频编码为h264格式(mp4)
    ...
    public void receiveMediaProcessTask(String message) {
        ...
        if (mediaFile==null) {
            LOGGER.error("从xc_media查询的媒体文件为空,message是:{}",message);
            return;
        }
        ...
        if (!result.equalsIgnoreCase("success")) {
            //记录错误信息
            LOGGER.error("生成mp4失败,video_path is{},error is {}",video_path,result);
            //更新处理状态为失败
        ...
        if (!m3u8_result.equalsIgnoreCase("success")) {
            //更新处理状态为失败
            mediaFile.setProcessStatus("303003");
            mediaFileRepository.save(mediaFile);
            return;
